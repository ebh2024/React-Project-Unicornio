...
const EditUnicornView = ({ visible, onHideDialog, unicornData }) => {
  const { updateUnicorn } = useContext(UnicornContext);
  ...
  const handleSubmit = useCallback(async (values, { resetForm }) => {
            try {
      await updateUnicorn(values._id, values);
              handleSuccess("Unicornio actualizado correctamente.");
              onHideDialog();
            } catch (error) {
      const errorMessage = error?.message || "Hubo un problema al actualizar el unicornio.";
      handleError(errorMessage);
            }
  }, [updateUnicorn, handleSuccess, handleError, onHideDialog]);
          ...
          {({ errors, touched }) => (
            <Form className="p-fluid">
              ...
              <div className="flex justify-content-end mt-4">
                <Button type="submit" label="Guardar Cambios" icon="pi pi-save" className="p-button-success" />
              </div>
            </Form>
          )}
        </Formik>
...
function areEqual(prevProps, nextProps) {
  return (
    ...
    JSON.stringify(prevProps.unicornData) === JSON.stringify(nextProps.unicornData)
  );
}
export default memo(EditUnicornView, areEqual);
